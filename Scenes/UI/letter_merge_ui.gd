...
func _input(event):
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_RIGHT:
		var starting_rot = UiInteractionManager.current_letter_held_rot
		var final_rot = starting_rot + 90
		var tween = get_tree().create_tween()
		tween.tween_property(UiInteractionManager.current_letter_held, "rotation", deg_to_rad(final_rot), 0.3).set_trans(Tween.TRANS_CUBIC)
...
func _on_letter_chosen(text):
	var draggable_letter = letter_scene.instantiate() as Label
	draggable_letter.text = text
	
	var letter_center = draggable_letter.size * 0.5
	draggable_letter.pivot_offset = Vector2(letter_center.x, letter_center.y)
...
func _on_go_to_game_button_down():
